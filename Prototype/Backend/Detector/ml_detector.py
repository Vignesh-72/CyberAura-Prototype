...
def count_special_chars(url):
    """Counts the number of common special characters in a URL."""
    url = str(url)
    special_chars = ['/', '?', '.', '=', '-', '&', '%', '#']
...
def run_ml_phase(df: pd.DataFrame) -> pd.DataFrame:
    ...
    unlabeled_df['prediction'] = predictions
    ml_detected_indices = unlabeled_df[unlabeled_df['prediction'] == 1].index
    df.loc[ml_detected_indices, 'attack_type'] = "ML Detected Malicious"
    
    print(f"[+] ML Phase complete. Found {len(ml_detected_indices)} new potential attacks.")
    return df
